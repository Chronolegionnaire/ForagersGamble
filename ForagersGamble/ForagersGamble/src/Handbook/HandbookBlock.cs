...
public class HandbookBlock
{
    ...
    public void OnAnyHotkey(string hotkeyCode, KeyCombination comb)
    {
        ...
        string codeKey = null;
        {
            ...
            {
                if (TryResolveFruitFromFruitTreeBlock(asBlock, stack, out var fruitFromAttrs2) &&
                    fruitFromAttrs2 != null)
                {
                ...
                if (!knownEnough)
                {
                    if (PlantKnowledgeUtil.TryResolveReferenceFruit(capi, asBlock, stack, out var fruitRef2) &&
                        fruitRef2 != null)
                    {
                        var fkey = ForagersGamble.Knowledge.ItemKey(fruitRef2);
                        if (!string.IsNullOrEmpty(fkey) && ForagersGamble.Knowledge.IsKnown(player.Entity, fkey))
                            knownEnough = true;
                    }
                }
        ...
        if (string.IsNullOrEmpty(codeKey))
        {
            codeKey = ForagersGamble.Knowledge.ItemKey(stack);
        }

        bool isInUnknownList = false;
        if (!string.IsNullOrEmpty(codeKey))
        {
            isInUnknownList = ForagersGamble.Knowledge.IsInUnknownUniverse(codeKey);
        }

        if (knownEnough || isInUnknownList) return;
