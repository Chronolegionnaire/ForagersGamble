    ...
    public static class Patch_Base_GetContentInfo_UnknownLiquid
    {
        ...
        static bool Prefix(BlockLiquidContainerBase __instance, ItemSlot inSlot, StringBuilder dsc, IWorldAccessor world)
        {
            ...
            var mGetCurrentLitres = AccessTools.DeclaredMethod(
                typeof(BlockLiquidContainerBase), "GetCurrentLitres", new[] { typeof(ItemStack) });
            var mGetContent = AccessTools.DeclaredMethod(
                typeof(BlockLiquidContainerBase), "GetContent", new[] { typeof(ItemStack) });

            var mGetContentInDummySlot = AccessTools.Method(
                typeof(BlockLiquidContainerBase), "GetContentInDummySlot", new[] { typeof(ItemSlot), typeof(ItemStack) });
            var mAppendPerishableInfoText = AccessTools.Method(
                typeof(BlockLiquidContainerBase), "AppendPerishableInfoText",
                new[] { typeof(ItemSlot), typeof(StringBuilder), typeof(IWorldAccessor), typeof(TransitionState), typeof(bool) });

            if (mGetCurrentLitres == null || mGetContent == null || mGetContentInDummySlot == null || mAppendPerishableInfoText == null)
                return true;
            ...
            {
                nameToken = Lang.Get(content.Collectible.Code.Domain + ":incontainer-" +
                                     content.Class.ToString().ToLowerInvariant() + "-" +
                                     content.Collectible.Code.Path);
            }
            ...
            ItemSlot dummySlot = (ItemSlot)mGetContentInDummySlot.Invoke(__instance, new object[] { inSlot, content });
            ...
            if (states != null && !dummySlot.Empty)
            {
                ...
                {
                    var ret = mAppendPerishableInfoText.Invoke(__instance, new object[] { dummySlot, dsc, world, state, nowSpoiling });
